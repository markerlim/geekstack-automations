...
class YuyuTeiScraper:
    """Generic scraper for a Yuyu-Tei (yuyu-tei.jp) single-card-sales shop page.

    Every TCG Yuyu-Tei sells runs the same shop template, so one scraper
    handles them all - only the base URL, Mongo collection, and backup
    filename prefix differ per game.
    """

    def __init__(self, base_url, collection_name, backup_prefix,
                 headless=True, db_name='geekstack', backup_dir='yuyuteidb'):
    ...
    def extract_all_links(self):
        ...
        Some games' sidebars re-list a set under multiple groups (e.g. a
        "newest" group and its normal group) using the same URL with
        different '#fragment's. Both load identical page content, so we
        dedupe by the fragment-stripped URL to avoid crawling a page twice.
        """
        try:
            # Get the page source
            ...
            # Find all sub-child corner divs within the accordion (any nesting depth)
    ...
    def save_backup(self, backup_filename=None):
        ...
        try:
            ...
            if not backup_filename:
                ...
                backup_filename = f'{self.backup_prefix}{timestamp}.json'

            backup_filepath = f"{self.backup_dir}/{backup_filename}"
    ...
    def load_from_backup(self, backup_filename=None):
        ...
        try:
            if not backup_filename:
                ...
                backups = [f for f in os.listdir(self.backup_dir) if f.startswith(self.backup_prefix)]
                if not backups:
...
def run_cli(description, scraper_factory, script_path, argv=None):
    """Shared CLI entrypoint for a game-specific Yuyu-Tei scraper.

    Args:
        description: argparse description shown in --help.
        scraper_factory: callable(headless: bool) -> a configured YuyuTeiScraper.
        script_path: command shown in retry usage hints.
        argv: argument list to parse instead of sys.argv (e.g. leftover args
            after a caller has already consumed a --game flag).
    """
    ...
    if args.upload_backup or args.use_latest_backup:
        ...
        _retry_upload_from_backup(scraper_factory, backup_filename, script_path)
        return

    # Normal scrape mode
    scraper = scraper_factory(headless=False)  # Set to False to see the browser
...
def _retry_upload_from_backup(scraper_factory, backup_filename, script_path):
    """Retry MongoDB upload from a backup file without re-scraping"""
    print("🔄 Retrying upload from backup...")

    scraper = scraper_factory(headless=True)  # Headless since we're not scraping
